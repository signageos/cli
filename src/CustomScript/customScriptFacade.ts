...
export async function uploadCode({
	...
	const codeArchive = await generateCustomScriptPlatformCodeArchive(workDir, config, platform);
 ...
 *
 * Archive is generated and stored in the .sos/upload directory.
 * After archive isn't needed anymore, it should be deleted using the delete method.
 */
async function generateCustomScriptPlatformCodeArchive(workDir: string, config: PlatformConfig, platform: string): Promise<CodeArchive> {
	...
	const md5Checksum = await getFileMD5Checksum(archiveFilePath, 'hex');
...
/**
 * Ensure that the .sos/upload directory exists in the current working directory.
 *
 * This directory is used to store temporary files generated by the CLI.
 */
async function ensureUploadDirectory() {
	const sosDir = await ensureSosDirectory();
	const uploadDir = path.join(sosDir, 'upload');
	await fs.ensureDir(uploadDir);
	return uploadDir;
}

/**
 * Ensure that the .sos directory exists in the current working directory.
 *
 * This directory is used to store temporary files generated by the CLI.
 */
async function ensureSosDirectory() {
	const SOS_DIR = '.sos';
	const README_FILE_PATH = path.join(SOS_DIR, 'README.txt');

	await fs.ensureDir(SOS_DIR);

	if (!(await fs.pathExists(README_FILE_PATH))) {
		await fs.writeFile(
			README_FILE_PATH,
			'This directory contains files generated by signageOS CLI. Do not modify. It should be ignored by version control.',
		);
	}
